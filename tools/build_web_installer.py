...
DEFAULT_REPO = "figamore/FluidMonitor"
...
def request_json(url: str, token: str | None = None) -> dict[str, Any]:
    headers = {
        ...
        "User-Agent": "fluidmonitor-installer-builder",
    }
    if token:
...
def download_asset(asset: dict[str, Any], dest: Path, token: str | None = None) -> None:
    headers = {"User-Agent": "fluidmonitor-installer-builder"}
    if token:
        headers["Authorization"] = f"Bearer {token}"
...
def main() -> int:
    ...
    parser.add_argument("--repo", default=DEFAULT_REPO, help="GitHub repository, e.g. figamore/FluidMonitor")
